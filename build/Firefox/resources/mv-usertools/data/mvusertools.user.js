...
var is_dark = jQuery("link[rel='stylesheet']").filter(function() {
...
var utpatchnotes = '<p style="font-size: 16px; font-weight: bold;">Actualización ' + utversion + "</p><br /><br />																- Corrección de errores.<br /><br />																";
...
jQuery(function() {
    if (utTagsOpcion == "si" || utTagsOpcion == undefined) {
        ...
        jQuery(":not(form)> div.post > div.autor > dl > dt > a").each(function() {
            ...
            jQuery(this).closest(".autor").children(".ut_tag_info").on("submit", "form.ut_tag_form", function() {
    ...
    if (utfiltrarfavs == "si" || utfiltrarfavs == undefined) {
        ...
        jQuery(document).on("mouseover", "body", function() {
            ...
            jQuery("#ut-filtros-fav a.foroicon").each(function() {
                ...
                if (utforosUnicos[interiorA]) jQuery(this).remove(); else utforosUnicos[interiorA] = true;
            });
        });
        ...
        jQuery(document).on("click", "#ut-filtros-fav a.foroicon", function() {
            ...
            jQuery("#ut-filtros-tags a.cat2").each(function() {
                ...
                if (utCatsUnicos[interiorB]) jQuery(this).remove(); else utCatsUnicos[interiorB] = true;
            });
        });
...
jQuery(function() {
    if (utforosfavs == "si" || utforosfavs == undefined) {
        ...
        var forosFavUpdate = function() {
            ...
            var forosFavDibujo = function() {
                for (i = 0; i < forosFav.length; i++) {
                    ...
                    jQuery("#ut-foros-fav").append(jQuery("<li>").html('<a href="/foro/' + forosFav[i] + '"><i class="ifid fid_' + forosFav[i] + '"></i></a><div class="ut-foros-fav-borrar"><i class="sprite UT-trash"></i></div>'));
                }
            };
            forosFavDibujo();
        };
        ...
        jQuery("div.fpanel div.icon").hover(function() {
        ...
        jQuery(".ut-foro-fav-add").click(function() {
            ...
            jQuery(this).closest("div.icon").find("i.ifid").each(function() {
                ...
                if (jQuery.inArray(foroNumber, forosFav) > -1) {
                    ...
                    localStorage["ut-forosFav"] = JSON.stringify(forosFav);
                    jQuery("#ut-foros-fav").append(jQuery("<li>").html('<a href="/foro/' + foroNumber + '"><i class="ifid fid_' + foroNumber + '"></i></a><div class="ut-foros-fav-borrar"><i class="sprite UT-trash"></i></div>'));
                }
            });
        ...
        function sticky_relocate() {
            ...
            if (window_top > div_top) jQuery("#foros-fav-float").addClass("foros-fav-float-sticky"); else jQuery("#foros-fav-float").removeClass("foros-fav-float-sticky");
        }
...
jQuery(document).ready(function() {
    ...
    var updateMacros = function(store, $container) {
        ...
        for (title in store) {
            if (!(title in macros)) {
                ...
                var $title = jQuery("<a>").html(' <a style="cursor:pointer;" title="Borrar macro" class="ut-remove-macro"><i class="sprite UT-trash-orange"></i></a>').prepend($spantitle).append($spanmacro);
                // solo +title+ para la lista de titulos
                var $item = jQuery('<li class="ut-titleymacro">').data("macro", title).append($title).hide();
    ...
    var updateMacrosButton = function(store, $container2) {
        ...
        for (title in store) {
            if (!(title in macros)) {
                ...
                var $item = jQuery('<li class="ut-titleymacro-list">').data("macro", title).append($title).hide();
    ...
    jQuery(function() {
        ...
        $macros.on("click", "a.ut-remove-macro", function() {
            ...
            // Si titulo es A y macro es B: macros['A'] te responderá 'B'
            storeJSON("macros", macros);
            updateMacros(macros, $macros);
...
jQuery(function() {
    if (utfavicon == "si" || utfavicon == undefined) {
        if (utnoti === undefined) {
            ...
            var utnoti_int = parseInt(jQuery('#userinfo a[href^="/foro/favoritos"] strong.bubble').html(), 10);
        }
        if (utavisos === undefined) {
            var utavisos_int = parseInt(0, 10);
        } else {
            var utavisos_int = parseInt(jQuery('#userinfo a[href^="/notificaciones"] strong.bubble').html(), 10);
        }
        if (utmsj === undefined) {
            var utmsj_int = parseInt(0, 10);
        } else {
            var utmsj_int = parseInt(jQuery('#userinfo a[href^="/mensajes"] strong.bubble').html(), 10);
        }
...
jQuery(function() {
    if (utlinksfooter == "si" || utlinksfooter == undefined) {
        ...
        } else if (jQuery('.live_link a[href^="/foro/live.php?tid="]').length > 0) {
            // Live
        ...
        jQuery("form#postear").sisyphus({
            ...
            onSave: function() {
                jQuery(utavisopostguardado).insertAfter('form#postear div[style="width: 410px"]').fadeIn("slow", function() {
                    jQuery(this).delay(2e3).fadeOut("slow", function() {
                        jQuery(this).delay(1e3).remove();
                    });
                });
            }
        });
...
jQuery("a[href^='/id/']").each(function() {
...
jQuery(".autor").each(function() {
    jQuery(this).append("<div class='usertools'>						<div class='online-pos'><a class='tooltip ut-offline sprite' href='http://www.mediavida.com/id/" + jQuery(this).data("name") + "' original-title='Perfil' ></a></div>						<div class='mensaje-pos'><a class='tooltip mensaje sprite' href='http://www.mediavida.com/mensajes/nuevo/" + jQuery(this).data("name") + "' original-title='Mensaje'></a></div>						<div class='firma-pos'><a class='tooltip ut-firma sprite' href='http://www.mediavida.com/id/" + jQuery(this).data("name") + "/firmas' original-title='Firma'></a></div>						<div class='blacklist-pos'><a class='tooltip blacklist blacklist-off sprite' href='javascript:void(0)' original-title='Blacklist'></a></div>				</div>");
});
...
jQuery(".autor").each(function() {
    ...
    } else jQuery(this).parent().prev().hide();
});
...
jQuery(".blacklist").click(function() {
    ...
    if (localStorage.getItem(localvalue) == "true") localStorage.removeItem(localvalue); else localStorage.setItem(localvalue, "true");
    ...
    jQuery(".autor").each(function() {
        ...
        } else if (localStorage.getItem(localvalue) == "true") {} else {}
    });
});
...
Mousetrap.bind("ctrl+alt+e", function() {
    document.location = BASE_URL + "/foro/favoritos";
});

//Go to your profile
Mousetrap.bind("ctrl+alt+q", function() {
    document.location = BASE_URL + "/id/" + utusername;
});

//Go to warnings
Mousetrap.bind("ctrl+alt+w", function() {
    document.location = BASE_URL + "/notificaciones";
});

//Go to private messages
Mousetrap.bind("ctrl+alt+r", function() {
    document.location = BASE_URL + "/mensajes";
});

//Go to forums
Mousetrap.bind("ctrl+alt+a", function() {
    document.location = BASE_URL + "/foro";
});

//Go to spy
Mousetrap.bind("ctrl+alt+d", function() {
    document.location = BASE_URL + "/foro/spy";
});